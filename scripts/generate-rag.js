...
function resolveType(typeNode) {
  ...
  switch (typeNode.type) {
    ...
    case "mapped":
      return "object";
    case "indexedAccess":
      return `${resolveType(typeNode.objectType)}[${resolveType(typeNode.indexType)}]`;
    default:
...
function renderEnum(enumNode, moduleName) {
  const comment = getCommentText(enumNode.comment);
  const lines = [];
  ...
  if (enumNode.children && enumNode.children.length > 0) {
    lines.push("", "## Values", "");
    for (const member of enumNode.children) {
      const memberComment = getCommentText(member.comment);
      const value = member.type ? resolveType(member.type) : "";
      lines.push(`- **${member.name}**${value ? ` = \`${value}\`` : ""}${memberComment ? ` — ${memberComment}` : ""}`);
    }
  }
...
for (const mod of doc.children || []) {
  ...
  if (moduleComment || (mod.children && mod.children.length > 0)) {
    const overviewLines = [];
    overviewLines.push(`# ${moduleName} — Module Overview`);
    if (moduleComment) overviewLines.push("", moduleComment);
    ...
    const enumNames = (mod.children || [])
      .filter((c) => c.kind === KIND.Enum)
      .map((c) => c.name);
    if (enumNames.length > 0) {
      overviewLines.push("", "## Enums", "");
      enumNames.forEach((n) => overviewLines.push(`- ${n}`));
    }
  ...
  for (const child of mod.children || []) {
    ...
    } else if (child.kind === KIND.Enum) {
      const content = renderEnum(child, moduleName);
      const filename = `${moduleSlug}-${slugify(child.name)}-enum.md`;
      writeChunk(filename, content);
      manifest.push({ file: filename, title: `${moduleName} > ${child.name} (enum)`, type: "enum" });
